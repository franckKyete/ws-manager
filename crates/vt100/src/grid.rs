...
impl Grid {
    ...
    pub fn set_size(&mut self, size: Size) {
        ...
        let target_rows = usize::from(size.rows);
        let cur_rows = self.rows.len();

        if target_rows < cur_rows {
            // Shrink rows: only push populated rows above the active cursor to scrollback
            let active_rows = (usize::from(self.pos.row) + 1).min(cur_rows);
            if active_rows > target_rows {
                let to_push = active_rows - target_rows;
                for _ in 0..to_push {
                ...
                self.pos.row = self.pos.row.saturating_sub(to_push as u16);
                self.saved_pos.row = self.saved_pos.row.saturating_sub(to_push as u16);
            }
            // Truncate remaining bottom unused rows to target_rows
            self.rows.truncate(target_rows);
        } else if target_rows > cur_rows {
            // Expanding rows (e.g. 15 -> 30): pull rows from scrollback to restore previous lines losslessly
            let diff = target_rows - cur_rows;
            for _ in 0..diff {
